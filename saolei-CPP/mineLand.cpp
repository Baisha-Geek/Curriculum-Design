...
mineLand::mineLand(QWidget* parent):QDialog(parent)
{
    ...
    lcdNum->setPalette(p);
    lcdNum->setStyleSheet ("background-color: white");
    ...
    lcdNum2->setStyleSheet("background-color: white");
    lcdNum2->setWindowFlags(Qt::FramelessWindowHint);
    lcdNum2->setWindowOpacity(0.5);

    emoctionLabel->setPixmap(QPixmap(":/images/jingxing.png").scaled(30,30,Qt::KeepAspectRatio));

    levels = 20;  //默认难度等级为最简单
    for(int i = 0 ; i < 225 ; ++i)
    ...
    create_mine();
    setWindowIcon(QIcon(":/images/mine.jpg"));
...
QVector<int> mineLand::Around(int x)
{
    QVector<int> around;
    int column = 15, row = 15;
    if (x < column)
    {
        if (x == 0)
        {
            around.append(x + 1);
            around.append(x + column);
            around.append(x + column + 1);
            return around;
        }
        else if (x == column - 1)
        {
...
void mineLand::show_empty(int nu)
{
    ...
    //	return;   //越界就停止递归
    if ((*(type_button.begin() + nu)) > 0&&!(*(mines.begin() + nu))->is_click())
    {
        (*(mines.begin() + nu))->like_click();
        if ((*(type_button.begin() + nu)) == 9)
        {
            foreach (int a , Around(nu))
            {
                if ((*(type_button.begin() + a)) == 9)
                    show_empty(a);
                else
                    (*(mines.begin() + a))->like_click();
            }
        }
        else
        {
...
void mineLand::winner()
{
    ...
    {
        emoctionLabel->setPixmap(QPixmap(":/images/shengli.png").scaled(30,30,Qt::KeepAspectRatio));
        emit win();
        for(auto A : mines)
...
{
    emoctionLabel->setPixmap(QPixmap(":/images/shibai.png").scaled(30,30,Qt::KeepAspectRatio));
    for(auto A : mines)
